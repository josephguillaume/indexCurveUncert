jQuery(function($) {
  ...
  $.extend(integerTableValidator, {
	...
	return (key >= 48 && key <= 57) || key==45; // 0-9
    },
  ...
  $.extend(tableInputBinding, {
    ...
    unsubscribe: function(el) {
	  ...
	  data = message.data;
	  var tbody = $(el).parents('.tableinput-container').find('.tableinput>tbody');
	  resize(tbody, Math.max(1, data[0].length), data.length);

	  $(el).find('tr').each(function(j) {
              var divs = $(this).find('td>div');
	      divs.each(function(i, div) {
		  $(div).text(data[i][j-1]);
	      });
	  });
	  //Don't trigger change because it would cause resend to server
	  //Do trigger change if server uses isolate
	  tbody.parent().change();
	  //$(el).trigger('change');
      }
  });
